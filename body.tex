...
The decomposition does not show the large pre-answer divergence that appeared in earlier specifications that included self-directed accept events. With accepts excluded, the waiting-period coefficient is small and negative ($\beta_2 \approx -0.021$, $p < .01$) and the answer-arrival increment is small and positive ($\beta_4 \approx +0.019$, $p < .01$), summing to the null net effect above. We return to the answers-versus-comments split in Table~\ref{tab:outcome_decomposition} once the robustness decomposition completes.
