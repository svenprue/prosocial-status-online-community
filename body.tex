...
The results offer only qualified support for H1 and H2, and mixed support for H3. Relative to matched controls and to their own pre-question baseline, users who receive an answer show no detectable pooled elevation in subsequent helping once self-directed accept events are excluded from the outcome (summed DiD HR\,$=$\,0.998, 95\% CI [0.99, 1.01], $p = .74$). A modest elevation is concentrated among newcomers (HR\,$=$\,1.11 in the first week of tenure, $p < .001$) and is indistinguishable from null among the most experienced users (HR\,$=$\,0.995). We report the treatment effect as the summed post- versus pre-question contrast and decompose it into waiting-period and answer-arrival components; Table~\ref{tab:outcome_decomposition} (pending the robustness pass) will show whether answers or comments drive any residual signal.

Our study makes three contributions. First, using a design built to remove the activity confound that inflates observational estimates, we provide a better-identified short-run behavioral test: receiving an answer is weakly associated with subsequent helping on Stack Overflow, with a null pooled contrast once self-directed accepts are excluded. Second, we show that any residual association is concentrated among newcomers and absent among veterans --- the tenure heterogeneity is the most informative pattern in the data, though we cannot cleanly separate reciprocity from selection into being answered. Third, methodologically, we decompose the outcome by help type and enter the waiting period as an explicit term, making pre-answer divergence visible rather than silently absorbed into the treatment effect.
...
Prior observational studies of online reciprocity report mixed magnitudes. \citet{baker_paying_2014} and \citet{chen_why_2019} find positive associations between help received and subsequent helping; \citet{wasko_why_2005} and \citet{joyce_predicting_2006} find weak or null relationships. Our matched DiD estimates sit below the larger positive reports once self-directed accept events are excluded from the outcome (pooled summed DiD HR\,$\approx$\,1.0; newcomer HR\,$\approx$\,1.11), consistent with the view that designs that do not net out baseline activity overstate the effect. The tenure gradient --- modest elevation among newcomers, null among veterans --- offers one reconciliation of the mixed prior findings.
...
% =====================================================================
% NUMERICS refreshed from accept-excluded re-run (Jul 9, 2026). Pooled
% summed DiD is null; newcomer elevation is modest (~11\%). The large
% waiting-period coefficients from the earlier with-accepts run do not
% persist once accepts are excluded --- see Table~\ref{tab:outcome_decomposition}
% (generated when robustness completes) for the answers-vs-comments split.
% AUTHOR DECISION: how hard to foreground any residual pre-trend versus the
% null pooled effect is a framing call --- flagged at each turn below.
% =====================================================================
To test Hypothesis~1, we estimate the pooled Cox model across all experience levels (Table~\ref{tab:pooled_experienced}). The treatment effect --- the summed post- versus pre-question contrast ($\beta_2 + \beta_4$) --- is indistinguishable from null: treated users' post-answer helping hazard does not differ detectably from their pre-question baseline relative to matched controls (HR\,$=$\,0.998, 95\% CI [0.99, 1.01], $p = .74$). Hypothesis~1 therefore receives only weak support at the pooled level.

The decomposition does not show the large pre-answer divergence that appeared in earlier specifications that included self-directed accept events. With accepts excluded, the waiting-period coefficient is small and negative ($\beta_2 \approx -0.021$, $p < .01$) and the answer-arrival increment is small and positive ($\beta_4 \approx +0.019$, $p < .01$), summing to the null net effect above. A waiting-period term this close to zero indicates that the act of asking, by itself, did not shift treated users' helping during the wait; it does not rule out question-induced influences --- such as the increased profile visibility that a question generates --- that persist into the post-answer window (see Section~\ref{sec:limitations}). We return to the answers-versus-comments split in Table~\ref{tab:outcome_decomposition} once the robustness decomposition completes.

\input{analysis/output_tables/regression_all.tex}

Table~\ref{tab:outcome_decomposition} decomposes the outcome by help type (answers, comments, and accepts) to show where any reciprocity signal is cleanly identified versus where it is dominated by pre-answer activity selection.
% \input{analysis/output_tables/composite_outcome.tex}  % uncomment when robustness pass completes
...
To test Hypothesis~2, we estimate the model separately within each of the seven tenure buckets (Table~\ref{tab:main_results}; Figure~\ref{fig:strength_rec}). The summed treatment effect is largest for the newest users --- in the first week of tenure, treated users help at an approximately 11\% higher post-answer hazard (HR\,$=$\,1.11, 95\% CI [1.09, 1.12], $p < .001$) --- and is indistinguishable from null among users with more than six years on the platform (HR\,$=$\,0.995, 95\% CI [0.97, 1.02]). Intermediate tenure buckets show small negative or null summed effects. This pattern offers qualified support for Hypothesis~2: any reciprocity-like elevation is modest and concentrated among newcomers.

The decomposition does not reproduce the steep waiting-period gradient reported in earlier with-accepts specifications. For newcomers, both the waiting-period ($\beta_2 \approx +0.041$) and answer-arrival ($\beta_4 \approx +0.061$) components are small and positive, summing to the 11\% net effect; among veterans both components are near zero. % AUTHOR DECISION: this materially weakens the "reciprocity recruits newcomers" claim. Consider whether H2 should be reframed around anticipatory engagement, or supported with the fast-response subsample where the waiting period is negligible.
...
Hypothesis~3 predicted that faster responses would strengthen the reciprocity effect. Table~\ref{tab:speed_results} reports the summed DiD and its response-time moderation, estimated separately by tenure bucket. The net moderation of the treatment effect is the sum $\gamma + \delta$, just as the base effect is $\beta_2 + \beta_4$. In the accept-excluded specification, this net moderation is negative for most tenure buckets (longer response times are associated with a lower summed DiD), with small magnitudes: for example, newcomers show a summed interaction of $-0.006$ ($p < .10$) and users with one to six months show $-0.029$ ($p < .001$). The discrete-bin estimates (Table~\ref{tab:response_time_bins}, Figure~\ref{fig:interaction_effect}) likewise do not show a clean monotone pattern favoring fast answers. Taken together, the evidence for Hypothesis~3 is mixed, and we treat the response-time results as exploratory.
...
One possible interpretation of the response-time pattern draws on platform session dynamics: users who post a question may remain actively engaged while waiting, elevating helping during the waiting period regardless of whether an answer arrives. On this account, a fast answer might discharge that pending engagement rather than trigger graded reciprocal gratitude. This interpretation is speculative --- we do not observe session boundaries (logins, logouts, inactivity gaps) --- and a competing explanation is that users whose questions attract answers were more active to begin with. The discrete-bin pattern does not show a clean monotone relationship favoring fast answers once the summed DiD is read as the estimand.
...
The most direct implication is that platforms seeking to grow their contributor base should prioritize ensuring that new users receive answers to their questions. In absolute terms, the newcomer summed DiD hazard ratio of 1.11 (Table~\ref{tab:main_results}) translates to an additional helping-event risk of roughly 1.2 percentage points per observation window (Table~\ref{tab:absolute_effects}), with a conservative number-needed-to-treat of approximately 86: about 86 newcomer questions must receive an answer to yield one additional helping event to others in the window. The pooled effect across all tenure levels is indistinguishable from null once self-directed accept events are excluded from the outcome, so the design implication is not a platform-wide helping multiplier but a targeted newcomer onboarding channel.

Our results do not support a distinct response-time lever for design. The net response-time moderation of the summed DiD is small and often negative (Table~\ref{tab:speed_results}), so we do not recommend timing interventions built on it. The better-supported emphasis is answer receipt itself --- ensuring that newcomers' questions are answered at all, and that answer notifications are salient --- rather than optimizing for a particular response-time window.
...
With self-directed accept events excluded from the helping outcome, the large pre-answer divergence that dominated earlier specifications does not persist in the default composite (answers + comments to others). The pooled summed treatment effect is indistinguishable from null, and any newcomer elevation is modest ($\approx$\,11\% in hazard-ratio terms). We cannot cleanly separate a reciprocal response to receiving help from selection into being answered; Table~\ref{tab:outcome_decomposition} (once generated) tests whether that ambiguity is concentrated in comments versus answers.

Beyond identification, the design captures a behavioral association but cannot identify the psychological mechanism producing it. Gratitude, moral obligation, incentive displacement, and platform-session dynamics are all \textit{proposed} interpretations consistent with parts of the pattern; the data do not allow us to distinguish among them, and we do not observe session boundaries, reputation salience, or emotional states. A competing explanation for any non-linear response-time pattern is answer quality (score, acceptance, length), which we test in robustness analyses. Future work combining behavioral records with experience-sampling measures of emotional state or moral reasoning during platform use could help disentangle these pathways.
...
Generalized reciprocity is widely theorized as a mechanism sustaining cooperation on knowledge-sharing platforms, but observational evidence is limited by a persistent confound: active users are more likely both to seek help and to give it, regardless of whether their questions are resolved. Our matched difference-in-differences survival design confronts this confound directly. Once self-directed accept events are excluded from the helping outcome, the pooled treatment effect is indistinguishable from null (summed DiD HR\,$=$\,0.998), with only a modest elevation among newcomers (HR\,$=$\,1.11) and no detectable effect among veterans. Excluding accepts also removes the large pre-answer divergence that had dominated earlier specifications, underscoring how outcome definition shapes what observational designs appear to find. Receiving help may nudge newcomers toward helping others, but our estimates cannot, on their own, separate that story from selection into being answered; distinguishing the two remains a task for designs with exogenous variation in whether, and how quickly, help arrives.