...
\paragraph{Answer quality as an alternative explanation.} A natural concern, raised in review, is that response time proxies for answer quality: very fast answers are disproportionately terse or link-only, whereas answers that take longer to arrive tend to be fuller and more detailed, so it may be the quality of the answer --- not its timing --- that elicits reciprocal helping. We probe this directly by re-estimating the pooled speed specification with controls for two observable quality signals: whether the question's first answer was subsequently accepted, and its vote score (Table~\ref{tab:answer_quality_robustness}). The controls do not merely attenuate the effect; they reverse it. The base treatment effect falls from a hazard ratio of 1.10 without quality controls to 0.96 with them, and the same reversal appears within the newcomer stratum, where the quality-controlled hazard ratio is 0.91 (Appendix Table~\ref{tab:newcomer_robustness}). Because answer acceptance and score are themselves post-treatment and partly endogenous to response speed, we read this as a bounding exercise rather than a clean causal decomposition. It nonetheless shows that the modest positive association we estimate is not robust to even coarse adjustment for answer quality, and that quality is a serious rival to any timing- or gratitude-based reading of the response-time pattern. We could not control for answer length, since the answer body text is dropped in preprocessing and would require re-parsing the raw data dump; that residual quality dimension is left uncontrolled and noted as a limitation.
