...
The revision-data estimates support H1 and H3 in the fast-response subsamples, while tenure moderation is weaker than in the prior sample. In the pooled Cox model, the post-answer treatment indicator is negative and highly significant (coefficient\,=\,$-$1.24, HR\,=\,0.29, 95\%~CI\,=\,[0.29, 0.29]), reflecting elevated helping among treated users during the waiting period and a sharp drop when the answer arrives. The discrete response-time-bin analysis shows positive reciprocity for answers arriving within 30 minutes (0--15 min: HR\,$\approx$\,1.08; 15--30 min: HR\,$\approx$\,1.21), with attenuation at longer delays. Speed interactions are positive across experience groups in the revision data ($\gamma$\,$\approx$\,0.05--0.33), indicating that longer response times amplify the post-answer contrast; we interpret this jointly with the inverted-U bin pattern in the results.
...
Table~\ref{tab:pooled_experienced} presents the pooled Cox model estimates across all experience levels. The coefficient on \textit{Received Answer $\times$ Post-Answer Received} is negative and highly statistically significant ($p < .001$). The estimated hazard ratio is 0.289 (95\%~CI\,=\,[0.29, 0.29]), indicating a lower instantaneous helping hazard for treated users in the post-answer active phase, conditional on the other time-varying terms. The waiting-period coefficient (\textit{Received Answer $\times$ Post-Question}) is large and positive (1.386, $p < .001$), indicating substantially elevated helping among treated users while their question remains unanswered. These patterns motivate the response-time subsample analysis below: positive reciprocity is concentrated in faster-response bins even though the pooled post-answer indicator is negative.
...
To test Hypothesis~2, we estimate the Cox model separately for each of the seven tenure buckets. Table~\ref{tab:main_results} reports the estimates, and Figure~\ref{fig:strength_rec} plots the corresponding hazard ratios. The post-answer treatment indicator is statistically significant in all seven tenure buckets, with hazard ratios clustered between 0.26 and 0.30 (e.g., newcomers: coefficient\,=\,$-$1.358, HR\,=\,0.26, 95\% CI\,=\,[0.25, 0.26], $p < .001$; users with more than six years: coefficient\,=\,$-$1.240, HR\,=\,0.29, [0.28, 0.29], $p < .001$). Unlike the prior sample, the revision data do not show a clear attenuation to a null effect among the most experienced users. Waiting-period coefficients remain large and positive across strata (e.g., HR\,$\approx$\,5.58 for newcomers), consistent with elevated engagement while treated users await an answer.
...
Because we observed that the control group helped more than the treatment group immediately after the question time (Figure~\ref{fig:help_rate_pooled}), we suspect that users with very short response times will exhibit distinct treatment effects, potentially because some exit the platform immediately after receiving an answer. To probe this mechanism, Figure~\ref{fig:interaction_effect} plots the treatment effect (expressed as a hazard ratio) by discrete response time bins, pooled across all tenure groups. For the fastest responses, the effect is positive (0--15 min: HR\,$\approx$\,1.08, $p < .001$; 15--30 min: HR\,$\approx$\,1.21, $p < .001$). The effect drops sharply for questions answered in 30--60 minutes (HR\,$\approx$\,0.07, $p < .001$) and remains below one at longer delays (e.g., 1--2 hr: HR\,$\approx$\,0.06; $>$3 days: HR\,$\approx$\,0.25). Table~\ref{tab:response_time_bins} reports the full bin-level estimates.
...
We interpret the non-linear pooled pattern through platform session dynamics. When a user posts a question, they enter a state of active, engaged waiting: they remain on the platform, browsing other questions, during the interval before their answer arrives. A very fast answer (under 30 minutes) can resolve this pending state while the user is still active, and the discrete-bin estimates show positive reciprocity in the 0--30-minute range (HR\,$\approx$\,1.08--1.21). For slower answers, the post-answer active indicator turns sharply negative in both the pooled Cox model and the 30+ minute bins, consistent with session exit or reduced engagement after long waits. At longer delays, the question's salience has faded, and the answer is less likely to prompt re-engagement at all. These dynamics produce the pattern in Figure~\ref{fig:interaction_effect}: positive effects in the fastest bins, a sharp drop in the 30--60-minute range, and attenuation at longer delays. The pattern is also consistent with the positive continuous speed interactions in Table~\ref{tab:speed_results}, since longer response times shift mass into bins with more negative post-answer contrasts. 
...
The theoretical account developed in this paper treats generalized reciprocity not as a uniform driver of community cooperation, but as a mechanism whose salience varies with a user's position on the trajectory from newcomer to established contributor. In the revision data, the post-answer treatment indicator remains statistically significant across all tenure buckets, with hazard ratios clustered around 0.26--0.30, rather than attenuating to a null effect among the most experienced users. Positive reciprocity in the fast-response subsamples (Table~\ref{tab:response_time_bins}) nonetheless remains most interpretable as a newcomer-facing recruitment mechanism.
...
The response time analysis qualifies the prediction, derived from laboratory-based theories of gratitude, that faster help should straightforwardly produce stronger reciprocity. The continuous interaction models in the revision data show positive speed interactions across experience groups: longer response times are associated with more negative post-answer contrasts in the speed-augmented specification. The discrete-bin analysis reveals a different non-linearity: positive reciprocity in the 0--30-minute bins (HR\,$\approx$\,1.08--1.21), a sharp drop in the 30--60-minute bin (HR\,$\approx$\,0.07), and attenuation at longer delays.

We interpret this as reflecting a re-engagement window created by the act of posting a question. When a user posts a question, they enter a state of active, engaged waiting: they remain on the platform, browsing other questions, during the interval before their answer arrives. Fast answers can arrive while that session is still active, and the 0--30-minute bins show positive treatment effects. Slower answers align with negative post-answer contrasts in the pooled model, consistent with session exit or reduced engagement after long waits. At longer delays, the question's salience has faded, and the answer is less likely to prompt re-engagement at all.
...
Generalized reciprocity is widely theorized as a mechanism sustaining cooperation on knowledge-sharing platforms, but observational evidence is limited by a persistent confound: active users are more likely both to seek help and to give it, regardless of whether their questions are resolved. Our matched DiD design addresses this confound. In the revision-data Cox models, the pooled post-answer indicator is negative (HR\,$\approx$\,0.29), while fast-response subsamples show positive reciprocity (HR\,$\approx$\,1.08--1.21 in the 0--30-minute bins). Reciprocity remains statistically detectable across tenure strata, with the strongest interpretive leverage for newcomer recruitment and fast-response windows. The response time findings add a structural qualification: positive effects concentrate in the fastest bins, drop sharply around 30--60 minutes, and attenuate at longer delays.