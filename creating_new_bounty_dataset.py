...
def create_user_answers_dataset(
   ...
   try:
       ...
       con.execute("""
           ...
           LEFT JOIN (
               SELECT post_id, bounty_amount 
               FROM votes 
               WHERE vote_type_id = 8
           ) b ON a.answer_id = b.post_id;
       """)
       ...
       # NEW: Create historical events - Answers received by users to their questions
       con.execute("""
       ...
       # NEW: Create historical events - Accepted answers received by users (renaming for clarity)
       con.execute("""
