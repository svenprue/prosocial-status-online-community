...
We note that this hypothesis follows from the dominant theoretical accounts of generalized reciprocity, which emphasize emotional immediacy and the salience of the helping experience. However, as we will show, field settings introduce structural dynamics --- particularly the behavioral consequences of platform session structure on task-driven platforms --- that qualify the predictions of laboratory-based theory. We return to this issue when presenting our results.
...
\caption{Treatment effect (hazard ratio) by response time bin, pooled across all tenure buckets. Each point shows the estimated hazard ratio for the treatment indicator within that response time bin, relative to the matched control. Error bars indicate 95\% confidence intervals. The dashed line at HR\,=\,1 indicates no effect. Very fast responses (under 30 minutes) yield no detectable reciprocity effect; the effect peaks at 30--60 minutes and attenuates for longer delays.}
\label{fig:interaction_effect}
\end{figure}

\paragraph{Response-time sweet spot.} We interpret the non-linear pooled pattern through platform session dynamics. Posting a question creates a period of heightened engagement: users remain active, browsing and encountering other questions. A very fast answer (under 30 minutes) resolves this pending state before the user has had meaningful time to browse; the reciprocity effect is suppressed not because gratitude is absent, but because the structural window for acting on it has barely opened. In the 30--60-minute range, the answer arrives while the session is still active and the question is still salient --- the sweet spot. At longer delays, the session has likely ended and the question lost relevance. These dynamics produce the inverted-U in Figure~\ref{fig:interaction_effect} and are consistent with the broadly negative continuous interactions, since most answers arrive after the 30-minute threshold.

Figure~\ref{fig:help_rate_adoption_pooled} provides descriptive support. Control users lie consistently below both treated-user lines, ruling out a simple ``still online'' explanation; the pending-question state elevates engagement above the control baseline. Once an answer arrives, the help rate converges toward control levels, consistent with the answer resolving the engaged-waiting state. Tenure-stratified versions of this figure are in Appendix Figure~\ref{fig:help_rate_adoption_by_tenure}.
...
The waiting-period $\times$ response-time interaction ($\delta = 0.0101$ to $0.0261$, all $p < .01$ across all strata) confirms that longer waits are associated with more helping \textit{during} the pending state --- the direct signature of sustained engagement. This holds even for the most established users whose base treatment effect is otherwise null. Question difficulty may provide a partial alternative account, as trivially easy questions resolved in seconds may generate less gratitude than those requiring genuine effort.
...
All three hypotheses receive support. The pooled reciprocity effect is modest but precisely estimated (HR\,=\,1.06), attributable specifically to answer receipt rather than to question-posting itself. The effect declines from HR\,$\approx$\,1.09 among the newest users to zero among those with more than six years of experience (H2). Speed interactions are consistently negative across tenure groups (H3), though the discrete-bin analysis adds a floor effect: very fast responses (under 30 minutes) produce no detectable boost, the effect peaks at 30--60 minutes, and it attenuates for longer delays, consistent with the response-time sweet spot account developed in the results.

%----------------------------------------------------------------------
\subsection{Identification and Measurement}
%----------------------------------------------------------------------

A prior contribution of this study is methodological. Prior estimates of generalized reciprocity in online communities are likely upward-biased: active users are more likely both to seek help and to provide it, so correlations between help received and help given conflate reciprocity with baseline user engagement. The matched difference-in-differences survival design addresses this confound; prior studies reporting positive correlations without such controls \citep{baker_paying_2014, chen_engaging_2018} may have captured differences in user engagement rather than causal reciprocity effects.
...
The response time findings provide qualified support for H3. Faster answers are generally better, consistent with emotional immediacy theory \citep{mccullough_is_2001, tsang_gratitude_2006}. The qualification is the floor effect at very short response times, which reveals that gratitude alone is insufficient to trigger reciprocal helping: users must also be present and browsing when the opportunity arises. This structural feature --- the engaged-waiting state created by a pending question --- has no natural analogue in laboratory settings where participants remain present throughout.

The speed moderation gradient across experience levels mirrors the attenuation in the base treatment effect: the interaction is strongest in the early-to-intermediate range ($|\gamma| \approx 0.06$ for 1W--6M) and disappears for the most established users. This parallel is consistent with the norm-displacement account: as community-specific motivations take hold, they weaken not only general reciprocal impulses but also the emotional sensitivity to response timing through which H3 operates.

Together, these results suggest that theories of emotional immediacy translate to field settings with a structural qualifier. Gratitude matters, but so does whether the user is still on the platform when it is time to act on it. Platform design choices --- such as surfacing unanswered questions to users at the moment their own answer arrives --- could maintain the structural conditions for reciprocal helping even when answers are delivered quickly.
...
Because the reciprocity effect is concentrated among newcomers, platforms seeking to leverage reciprocity for community growth should prioritize ensuring that newcomers receive answers at all. The base effect is substantial across response times, so ensuring answer receipt matters more than optimizing its speed. Questions from new users could be given visibility boosts in recommendation systems to increase the probability that they attract a response.
...
The response time findings are consistent with H3 --- faster responses generally produce stronger reciprocity --- but with a floor effect: the reciprocity boost is near zero for answers arriving in under 30 minutes, peaks at 30--60 minutes (HR\,$\approx$\,1.18), and attenuates thereafter. This response-time sweet spot reflects platform session dynamics: an answer that resolves a pending question before the user has browsed suppresses the structural conditions for reciprocal helping, even when gratitude is present.
