...
\multicolumn{1}{2}{Propensity score matching }\\
...
\input{output_tables/desc_stats}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{output_figures/help_rate_2panel.eps}
...
The figure serves three purposes. First, it provides a visual check on the comparability of the matched groups. In both panels, the treated and control lines overlap closely during the pre-question phase (days $-7$ to $0$), confirming that propensity score matching has successfully balanced baseline helping activity between the groups within the strata \footnote{This check also holds for the other groups but is not shown here}. This overlap is the graphical counterpart of the low standardized mean differences reported in Table~\ref{tab:balance} and supports the parallel trends assumption that underpins our difference-in-differences design.

Second, the figure provides a clear visual preview of the reciprocity effect documented formally below. Among newcomers (top panel), the treated and control lines begin to diverge after $T_Q$. Treated newcomers show a sustained elevation in help rate relative to controls in the post-answer phase, consistent with a reciprocity response triggered by receiving an answer. Among experienced users (bottom panel), no comparable divergence is apparent: the treated and control lines remain closely interleaved throughout the post-question period, foreshadowing the null treatment effect that the Cox models will confirm for these tenure buckets.

Third, both panels show a pronounced spike in helping activity around $T_Q$ for both treated and control users. This spike reflects active-session dynamics: users who are engaged enough to post a question are also more likely to be browsing and answering during the same session. The spike is particularly large for newcomers, who may be exploring the platform intensively during their initial visits. Importantly, this spike is \textit{symmetric} across treated and control groups, indicating that it reflects the act of being on the platform rather than the receipt of an answer. We can also see that the help rate for more senior users spikes around the time of the question, regardless of them receiving an answer or not. Their willingness to help is unconditional rather than non-responsive to reciprocity. 

%----------------------------------------------------------------------
\subsection{Main Results: The Reciprocity Effect (H1)}
%----------------------------------------------------------------------

\input{output_tables/main_results}

We begin with the primary test of generalized reciprocity (H1), formalizing the patterns visible in Figure~\ref{fig:help_rate_2panel}. The coefficient on \textit{isTreatedActive}---which captures the additional lift in helping behavior for treated users after receiving an answer, relative to matched controls---is positive across all tenure buckets and statistically significant for newcomers.

For newcomers with less than one week of tenure, the hazard ratio is 1.15 (95\% CI: [1.13, 1.17], $p < .001$), indicating that receiving an answer increases the instantaneous rate of helping by 15\%. The effect remains significant but is substantially attenuated for users with one week to one month of tenure (HR\,=\,1.03, 95\% CI: [1.01, 1.04], $p < .001$). For all intermediate tenure buckets---from one month through six years---the treatment effect is close to zero and statistically insignificant, with hazard ratios indistinguishable from 1.00. Interestingly, a small but significant effect re-emerges for users with more than six years of tenure (HR\,=\,1.02, $p < .01$), which we discuss further in the context of the response time analysis below.

The waiting period covariate (\textit{treatedPostQuestion}) tests whether treated users already display elevated helping after posting their question but before receiving an answer. For newcomers, this coefficient is positive and significant ($b = 0.262, p < .001$), indicating that users whose questions will eventually receive answers are already more active in the waiting period. This likely reflects active-session dynamics: newcomers who engage deeply enough to ask a well-formed question may also browse and answer others' questions during the same visit. Importantly, the treatment effect (\textit{isTreatedActive}) captures the \textit{additional} lift in helping that occurs specifically when the answer arrives, above and beyond this waiting period activity. For intermediate tenure buckets, the waiting period effect is small and mostly insignificant ($b = 0.004$, $p > .05$ for 1--6 months; $b = 0.005$, $p > .05$ for 6--12 months), though it becomes significant again for users with one to three years ($b = 0.006, p < .05$) and longer tenure, likely reflecting active-session dynamics among these users as well.

These results provide strong support for Hypothesis~1: receiving help increases a user's propensity to help others on the platform.

%----------------------------------------------------------------------
\subsection{Experience Moderation (H2)}
%----------------------------------------------------------------------

To test Hypothesis 2, we examine how the treatment effect varies across tenure buckets. Figure~\ref{fig:strength_rec} plots the hazard ratio associated with \textit{isTreatedActive} for each bucket.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{output_figures/strength_rec.pdf}
...
The reciprocity effect is strongest among users with less than one week of tenure (HR\,=\,1.15, $p < .001$) and declines sharply with experience. Users in the one-week-to-one-month bucket still show a significant but much smaller effect (HR\,=\,1.03, $p < .001$). For all buckets from one month through six years, the effect is statistically indistinguishable from zero, with hazard ratios between 1.000 and 1.003. This represents a striking decline: the 15\% increase in helping hazard observed for newcomers vanishes entirely within the first few months of platform tenure. The small but significant effect for users with more than six years of tenure (HR\,=\,1.02, $p < .01$) is an exception that we discuss in the context of the response time moderation analysis below.

These findings support Hypothesis 2. The generalized reciprocity effect is concentrated among newcomers and diminishes as users gain experience on the platform, consistent with the descriptive patterns in Figure~\ref{fig:help_rate_2panel} where the treated--control divergence is visible only for newcomers. This pattern is consistent with our theoretical account: newcomers, who have not yet internalized community-specific norms, are most susceptible to the moral and emotional mechanisms that drive reciprocity. As users become socialized, these mechanisms are displaced by community-specific incentive structures.
...
\input{output_tables/speed_results}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{output_figures/speed_moderation.pdf}
